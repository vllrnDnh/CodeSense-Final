...
export const HomeDashboard: React.FC = () => {
  ...
  const calculateProgress = () => {
    if (!user) return 0;
    return (user.currentLevel / 4) * 100;
  };

  const handleExit = () => {
    if (isGuest) {
      // Clear guest session and return to landing
      navigate('/');
    } else {
      logout();
      navigate('/');
  ...
  return (
    <div style={{
      ...
      {/* Header HUD: Standardized across zoom levels */}
      <header style={{
        display: 'flex',
      ...
      <div style={{
        width: '95%',
        maxWidth: '1280px', // Standard desktop sweet spot
        display: 'grid',
        // This ensures columns stack on smaller zoom/screens automatically
          ...
          <div style={heroSectionStyle}>
            ...
            <div style={{ position: 'relative', textAlign: 'center', zIndex: 1 }}>
              ...
              <h2 style={heroTitleStyle}>
                {isGuest ? 'Welcome, Guest!' : `Welcome back, ${user?.playerName || 'User'}!`}
              </h2>
              <p style={{ color: '#8b949e', fontSize: '16px', margin: 0 }}>
                {isGuest 
                  ? 'Explore the sandbox freely. Sign up to save your progress!'
                  : `Continue your journey to master code safety as a ${user?.characterType || 'squire'}.`}
              </p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <>
                <div style={statsBoxStyle}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <div style={statsLabelStyle}>CURRENT LEVEL</div>
                      <div style={statsValueStyle}>Lvl {user?.currentLevel || '1'}</div>
                    </div>
                ...
                <button onClick={() => navigate('/profile')} style={fullProfileBtnStyle}>View Full Profile</button>
              </>
            )}
          </div>
          ...
          <div style={sidebarCardStyle}>
            ...
            {isGuest ? (
              <div style={guestPlaceholderStyle}>
                <p>Sign up to track progress</p>
              </div>
            ) : (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ color: '#8b949e', fontSize: '12px' }}>Journey Progress</span>
                  <span style={{ color: 'white', fontSize: '12px', fontWeight: '600' }}>{calculateProgress()}%</span>
                </div>
                <div style={progressBarContainerStyle}>
                  <div style={{ width: `${calculateProgress()}%`, height: '100%', background: 'linear-gradient(90deg, #4caf50 0%, #66bb6a 100%)' }} />
                </div>
              </div>
            )}
          </div>
...
// --- Styles Objects to clean up JSX ---
